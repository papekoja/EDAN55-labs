...
pub fn calculate_cut(
    set_a: &HashMap<i32, Vec<(i32, i32)>>,
    set_b: &HashMap<i32, Vec<(i32, i32)>>,
) -> i32 {
    let mut sum: i32 = 0;
    ...
    for (&node_id, edges) in set_a {
        for &(neighbor_id, weight) in edges {
            let rearranged_edge = if node_id < neighbor_id {
                (node_id, neighbor_id)
            } else {
                (neighbor_id, node_id)
            };
...
pub fn swap(
    node_id: i32,
    set_a: &mut HashMap<i32, Vec<(i32, i32)>>,
    set_b: &mut HashMap<i32, Vec<(i32, i32)>>,
) {
